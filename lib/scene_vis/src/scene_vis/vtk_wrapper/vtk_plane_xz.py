...
class VtkXZPlane(vtk.vtkActor):
    ...
    def set_plane(self, plane_coeffs, xz_extents):
        """Calculates 3 points for plane visualization
        based on the provided ground plane

        Args:
            plane_coeffs: Plane equation coefficients (a, b, c, d)
        ...
        plane_point0 = self._calculate_plane_point_y(
            plane_coeffs, (min_x, None, min_z))
        plane_point1 = self._calculate_plane_point_y(
            plane_coeffs, (max_x, None, min_z))
        plane_point2 = self._calculate_plane_point_y(
            plane_coeffs, (min_x, None, max_z))
