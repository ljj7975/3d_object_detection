...
class VoxelGrid:
    def __init__(self):
        self.voxel_indices = np.asarray([])
        self.min_voxel_coord = [0, 0, 0]
        self.voxel_size = 1.0
...
class VtkVoxelGrid:
    ...
    def set_voxel_grid(self, voxel_grid: VoxelGrid):
    ...
    def set_scalar_array(self, scalar_name, scalars, scalar_range=None):
        ...
        if scalar_range is not None:
            ...
            if range_min == range_max:
                ...
                remapped_scalar_values = remapped_scalar_values.astype(np.float32)
        else:
            ...
            if scalar_min == scalar_max:
                remapped_scalar_values = np.full(scalars.shape, scalar_min, dtype=np.float32)
            else:
