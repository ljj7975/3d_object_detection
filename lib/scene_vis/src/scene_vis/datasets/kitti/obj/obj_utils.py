...
from scene_vis.core import box_3d_encoder
...
def _check_difficulty(obj, difficulty):
    ...
    return (
        (obj.occlusion <= OCCLUSION[difficulty])
        and (obj.truncation <= TRUNCATION[difficulty])
        and (obj.y2 - obj.y1) >= HEIGHT[difficulty]
    )
...
def filter_labels_by_box_2d_height(obj_labels, box_2d_height):
    ...
    """
    height_mask = [(obj_label.y2 - obj_label.y1) > box_2d_height for obj_label in obj_labels]

    return obj_labels[height_mask], height_mask


def filter_labels(
    obj_labels, classes=None, difficulty=None, box_2d_height=None, occlusion=None, truncation=None
):
...
def boxes_2d_from_obj_labels(obj_labels):
    return np.asarray(
        [box_3d_encoder.object_label_to_box_2d(obj_label) for obj_label in obj_labels]
    )


def boxes_3d_from_obj_labels(obj_labels):
    return np.asarray(
        [box_3d_encoder.object_label_to_box_3d(obj_label) for obj_label in obj_labels]
    )


def obj_classes_from_obj_labels(obj_labels):
    return np.asarray([obj_label.type for obj_label in obj_labels])
...
def get_lidar_point_cloud_for_cam(sample_name, frame_calib, velo_dir, image_shape=None, cam_idx=2):
...
def get_stereo_point_cloud(sample_name, calib_dir, disp_dir):
    ...
    stereo_calibration_info = calib_utils.get_stereo_calibration(frame_calib.p2, frame_calib.p3)

    # Read disparity
    disp = cv2.imread(disp_dir + "/{}.png".format(sample_name), cv2.IMREAD_ANYDEPTH)
...
def get_depth_map(sample_name, depth_dir):
    ...
    depth_map = DepthMap.from_file(depth_map_path)
    return depth_map
...
def get_depth_map_point_cloud(sample_name, frame_calib, depth_dir):
    ...
    depth_map_path = get_depth_map_path(sample_name, depth_dir)
    depth_map = DepthMap.from_file(depth_map_path)
    depth_map_shape = depth_map.data.shape[0:2]
    ...
    return depth_map.to_point_cloud(cam_info)

    # Calculate points from depth map
    depth_map_flattened = depth_map.flatten()
    xx, yy = np.meshgrid(np.arange(0, depth_map_shape[1], 1), np.arange(0, depth_map_shape[0], 1))
...
def points_in_img_filter(points_in_img, image_shape):
    ...
    image_filter = (
        (points_in_img[0] >= 0)
        & (points_in_img[0] < image_shape[1])
        & (points_in_img[1] >= 0)
        & (points_in_img[1] < image_shape[0])
    )
...
def compute_box_3d_corners(box_3d):
    ...
    # 3D BB corners
    x_corners = np.array([half_l, half_l, -half_l, -half_l, half_l, half_l, -half_l, -half_l])
    y_corners = np.array([0, 0, 0, 0, -h, -h, -h, -h])
    z_corners = np.array([half_w, -half_w, -half_w, half_w, half_w, -half_w, -half_w, half_w])
    corners_3d = np.dot(rot, np.array([x_corners, y_corners, z_corners]))
