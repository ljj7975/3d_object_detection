...
from scene_vis.datasets.kitti.obj import calib_utils, obj_utils
...
def main():
    ...
    if point_cloud_source not in ["lidar", "depth"]:
        raise ValueError("Invalid point cloud source {}".format(point_cloud_source))
    ...
    for frame_idx in range(*frame_range):
        ...
        if point_cloud_source == "lidar":
            ...
            velo_curr_points_padded = np.pad(
                velo_points, [[0, 0], [0, 1]], constant_values=1.0, mode="constant"
            )
            ...
            depth_maps_dir = core.data_dir() + "/depth_completion/raw/{}/depth_02_{}".format(
                drive_id, fill_type
            )
            ...
            cam0_curr_pc = depth_map.to_point_cloud(cam_info)
